...
if is_mac_os():
    ...
    #device = torch.device('cuda:0') #THIS CAUSES IT TO CRASH IF YOU DONT HAVE A NVIDIA GPU WITH CUDA FOR TORCH
    device = torch.device('cpu')

# Load model
tts = TTS(model_name=params["model_name"])#.to(device) #testing removing .to(device) since its meant to get called internally
...
def gen_voice(string, spk, speed, temperature, english): #test adding temp
    ...
    tts.tts_to_file(
        text=string,
        speed=speed,
        temperature=temperature,#test adding temp
...
with gr.Blocks() as app:
    ...
    with gr.Row():
        with gr.Column():
            ...
            #testing adding a temp slider
            temperature_slider = gr.Slider(label='Temperature', minimum=0.1, maximum=1.5, value=0.7, step=0.05)
    ...
    submit_button.click(
        fn=gen_voice,
        inputs=[text_input, speaker_dropdown, speed_slider, temperature_slider, language_dropdown], #test added temp
        outputs=audio_output
    )

if __name__ == "__main__":
    app.launch(share=False)#testing adding this param

#added temp for testing rule of thumb how its meant to be:
#0.6 - calm
#0.7 - natural default
#0.9 - more expressive
